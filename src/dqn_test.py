...
from utils import get_args, get_incoming_vehicles, get_green_phases, get_phases, get_sumo_cmd
...
def run_test(model_id):
    ...
    q_network.model = tf.keras.models.load_model(
        os.path.join(os.path.dirname(__file__), f"models/model_episode_{model_id}")
    )
    ...
    print(f"Total waiting time: {total_waiting_time}, Total reward: {total_reward}")

    return {"total_waiting_time": total_waiting_time, "total_reward": total_reward}


if __name__ == "__main__":
    run_test(MAX_EPISODES)
